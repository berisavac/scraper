...
import { getMatchListCache, getMatchDetailsCache, setCache, deleteCache, getCacheKey } from './cache.js';
const app = new Hono();
// Middleware
app.use('*', logger());
app.use('*', cors());
...
app.get('/scrape/all-leagues', async (c) => {
    ...
    try {
        ...
        const filteredMatches = matchList.matches.filter((match) => ALLOWED_LEAGUES.some((league) => match.league.toLowerCase().includes(league.toLowerCase())));
        console.log(`Scraping ${filteredMatches.length} matches from allowed leagues...`);
        // 3. Scrape detalje sa p-limit(3)
        const errors = [];
        const scrapedLeagues = new Set();
        const tasks = filteredMatches.map((match) => limit(async () => {
            try {
