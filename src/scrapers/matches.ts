...
async function scrapeMatchesWithRetry(): Promise<MatchSummary[]> {
...
async function scrapeMatches(): Promise<MatchSummary[]> {
  ...
  try {
    ...
    const scrapedData = await page.evaluate((allowedLeagues: string[]) => {
      const results: Array<{
        id: string;
        league: string;
      ...
      allElements.forEach(el => {
        ...
        if (el.classList.contains('event__match')) {
          ...
          const linkEl = el.querySelector('a.eventRowLink') as HTMLAnchorElement;
